...
filterByAccess = function (userId) {
  var user = getUser(userId);
  var filter = Roles.userIsInRole(user, "fullAccess")
    ? {}
    : filterByOwnership(userId);
...
requireUser = function (userId) {
  if (!isUser(userId)) {
    throw new Meteor.Error(
      "access-denied",
      "You must be logged in for that action."
    );
  }
};
...
requireOwnership = function (userId, doc) {
  if (!isOwner(userId, doc)) {
    throw new Meteor.Error(
      "access-denied",
      `You don't have ownership for this entity.`
    );
  }
};
...
requireAccess = function (userId, doc) {
  if (!hasAccess(userId, doc)) {
    throw new Meteor.Error(
      "access-denied",
      `You don't have access permissions for this entity.`
    );
  }
};
...
requireAdmin = function (userId) {
  if (!isAdmin(userId)) {
    throw new Meteor.Error(
      "access-denied",
      "You must have admin privileges for that action."
    );
  }
};
