function userId() { return FlowRouter.getParam('id') }
...
Template.userView.events({
    ...
    Accounts.changePassword(
      ...
      function(err) {
        if (err) {
          ...
          template.state.set('passwordSuccessMessage', 'Password changed successfully.');
    ...
    confirmModal('Sure you want to reset this users password?', function() {
      Meteor.call('resetUserPassword', userId(), (err, res) => {
        if (err) {
          ...
          template.state.set('adminMessage', `Password successfully reset to "${res}".`);
        }
      });
    });
  },
  ...
  'click .js-delete': function() {
    confirmModal('Are you really sure you want to delete this user?', function() {
