...
Meteor.publish("favoriteDashboards", function () {
  ...
  var filter = {
    $and: [{ _id: { $in: favorites } }, filterByAccess(this.userId)],
  };
  ...
  return {
    find: function () {
      return Dashboards.find({
        $and: [filterByAccess(this.userId), { _id: _id }],
      });
    },
    children: [
      {
        find: function (dashboard) {
          return Visualizations.find(
            { _id: { $in: _.pluck(dashboard.widgets, "visId") } },
            {
...
Meteor.publishComposite("job", function (_id) {
  return {
    ...
    children: [
      {
        find: (job) =>
          JobResults.find({ jobId: job._id, parameters: job.parameters }),
      },
      {
        find: (job) =>
          Visualizations.find(
...
Meteor.publish("visualizationsDropdown", function (
  includeNonOwned = false,
  search = "",
  limit = 10
) {
  var accessFilter = includeNonOwned
    ? filterByAccess(this.userId)
...
Meteor.publish("visualization", function (
  _id,
  parameters,
  dashboardId = false
) {
  ...
  if (dashboardId) {
    ...
    if (!_.contains(_.pluck(dashboard.widgets, "visId"), _id)) {
      throw new Meteor.Error(
        "access-denied",
        `Visualization ${_id} is not in dashboard ${dashboardId}`
      );
    } else {
  ...
  const results = JobResults.findOne({
    jobId: vis.jobId,
    parameters: parameters,
  });
  if (
    !results ||
    (results.expiresAt < new Date() && results.status !== "running")
  ) {
...
Meteor.publish("jobAlarms", function (options = {}) {
  ...
  var jobsCursor = Jobs.find(filterByAccess(this.userId), {
    fields: { name: 1 },
  });
